...
SELECT COMMON_BOARD.TITLE, COMMON_BOARD.CONTENT, COMMON_BOARD.DATE_CREATED, ATTACHED_FILES.FILE_FOLDER_NAME,
		ATTACHED_FILES.ORIGINALFILE_NAME, EVENT_START_DATE, EVENT_END_DATE
FROM MAIN_BOARD INNER JOIN COMMON_BOARD
ON MAIN_BOARD.BOARD_UID = COMMON_BOARD.BOARD_UID
INNER JOIN ATTACHED_FILES
ON COMMON_BOARD.COMMONBOARD_UID = ATTACHED_FILES.COMMONBOARD_UID
WHERE MAIN_BOARD.BOARD_TYPE = 'EVENT';
-- 이벤트 몇개나 화면에 뽑을건지에 따라 시간순으로 ORDER BY해서 상단 2개만 뽑아내거나
-- 하는 제약이 필요할 거 같아요
...
-- 하드코딩
SELECT COMMON_BOARD.TITLE, COMMON_BOARD.WRITER, COMMON_BOARD.CONTENT, COMMON_BOARD.DATE_CREATED, ATTACHED_FILES.FILE_FOLDER_NAME,
...
-- faqList
-- TITLE,CONTENT,BOARD_TYPE_DESCRIPTION
-- 게시글(FAQ)제목, 내용, 분류.
-- WHERE절 조건은 이대로 그냥 쓰면 됌.
SELECT COMMON_BOARD.TITLE, COMMON_BOARD.CONTENT ,MAIN_BOARD.BOARD_TYPE_DESCRIPTION
FROM COMMON_BOARD INNER JOIN MAIN_BOARD
ON COMMON_BOARD.BOARD_UID = MAIN_BOARD.BOARD_UID
WHERE COMMON_BOARD.BOARD_UID IN ('BOARD_FAQ_REGISTER','BOARD_FAQ_SHIPPING','BOARD_FAQ_MAP','BOARD_FAQ_ACCIDENT');



-- IdSearch -> IdResult_su
-- ID찾기에서 이름과 전화번호로 USER_UID가 결과값으로 나오면 IdResult_su로 이동.
-- 값이 비어있으면 IdResult_fa로 이동.
-- WHERE 절 조건 부분은 현재 하드코딩
SELECT USER_UID
FROM USERS
