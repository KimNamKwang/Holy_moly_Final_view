...
(
  ...
  REPLY_FOR_FAQ   VARCHAR(500) NULL     COMMENT 'FAQ답글',
  ...
  EVENT_DATE_UID  VARCHAR(100) NULL     COMMENT '이벤트기간UID',
  PRIMARY KEY (COMMONBOARD_UID)
) COMMENT '전체 게시판';

CREATE TABLE EVENT_DATE_BOARD
(
  EVENT_DATE_UID   VARCHAR(100) NOT NULL COMMENT '이벤트기간UID',
  EVENT_START_DATE DATETIME     NOT NULL COMMENT '이벤트시작일',
  EVENT_END_DATE   DATETIME     NOT NULL COMMENT '이벤트끝일',
  BOARD_UID        VARCHAR(100) NOT NULL COMMENT '게시판 타입UID',
  PRIMARY KEY (EVENT_DATE_UID)
);
...
(
  ...
  PROGRESS_STATUS_TIME        DATETIME     NULL     COMMENT '진행상황일시',
  TRACKING_NUMBER             VARCHAR(100) NULL     COMMENT '운송장번호',
...
ALTER TABLE EVENT_DATE_BOARD
  ADD CONSTRAINT FK_MAIN_BOARD_TO_EVENT_DATE_BOARD
    FOREIGN KEY (BOARD_UID)
    REFERENCES MAIN_BOARD (BOARD_UID);

ALTER TABLE COMMON_BOARD
  ADD CONSTRAINT FK_EVENT_DATE_BOARD_TO_COMMON_BOARD
    FOREIGN KEY (EVENT_DATE_UID)
    REFERENCES EVENT_DATE_BOARD (EVENT_DATE_UID);

ALTER TABLE PROGRESS_STATUS_TYPE
  ADD CONSTRAINT FK_SHIPMENT_TO_PROGRESS_STATUS_TYPE
    FOREIGN KEY (TRACKING_NUMBER)
    REFERENCES SHIPMENT (TRACKING_NUMBER);
